...
- Fixed shimmer and `LookingGlass` content being sized wrong when the app was opened in landscape. The screen size is captured once and was read from `UIScreen.main.bounds`, which reports the current interface orientation. It now comes from the screen's fixed coordinate space, whose bounds always reflect a portrait-up orientation, so the landscape swap is correct however the app was oriented at that moment.
