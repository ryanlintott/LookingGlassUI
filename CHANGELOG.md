...
- `MotionManager.updateInterval` and `MotionManager.disabled` now describe the effective app-wide motion configuration rather than any one scene's requested values. The built-in effects still respect the `updateInterval` and `disabled` values supplied by their own scene.
