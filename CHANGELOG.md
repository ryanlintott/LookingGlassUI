...
- Fixed shimmer and `LookingGlass` content being sized wrong in both orientations when the app first drew one of them in landscape. The screen size is captured once from `UIScreen.main.bounds`, which reports the current interface orientation, and the stored value is now normalised to portrait so the landscape swap is correct however the app was oriented at that moment.
