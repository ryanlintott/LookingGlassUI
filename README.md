...
`MotionManager` holds one scene's configuration and changes rarely. `DeviceMotion` holds everything that changes with the device — the rotation, the interface orientation, and the interval the shared service is running at — and changes on every motion update. If you want direct access to either you can read them from the environment if you have added a [`.motionManager()`](#motionmanager) modifier higher up in the hierarchy.
