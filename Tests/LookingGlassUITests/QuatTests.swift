...
final class QuatTests: XCTestCase {
    ...
    /// A zero angle rotation must be exactly equal to identity or the change check in `DeviceMotion.update(quaternion:)` would report a change every time the device passed through that quadrant.
